...
def get_edu_prog(plan_id: int, custom_conn):
    ...
    for subj in subjects:
        ...
        for inf_subj in a:
            ...
            ans[inf_subj.sem - 2]['disciplines'].append({
                'title': subj.subject,
                'desctiption': summar[:min(200, len(summar))],
                'ze': inf_subj.ze,
                'hours': inf_subj.hours
            })
            ans[inf_subj.sem - 2]['disciplines'].sort(key=lambda x: x["hours"], reverse=True)
            if inf_subj.exam:
                ans[inf_subj.sem - 2]['exams'].append(subj.subject)
            if inf_subj.offset:
                ans[inf_subj.sem - 2]['zaceths'].append(subj.subject)
            if inf_subj.diff_offset:
                ans[inf_subj.sem - 2]['div_zaceths'].append(subj.subject)
            if inf_subj.course_work:
                ans[inf_subj.sem - 2]['kursah'] = subj.subject
