...
def get_awesome_proff_sphere(plan_id: int, session):
    ...
    i = 0
    for x in spheres[:3]:
        a = session.query(FullVac).filter(FullVac.sphere == x[i]).all()[:3]
        nice_vacancy = []
        for y in a:
            tmp = {}
            tmp['name'] = y.exp
        ...
        ans.append({'title': x[i], 'vacansions':nice_vacancy})
        i += 1
    return ans
...
def get_edu_prog(plan_id: int, custom_conn):
    ...
    for subj in subjects:
        ...
        for inf_subj in a:
            ...
            if inf_subj.offset:
                ans[inf_subj.sem ]['zaceths'].append(subj.subject)
            if inf_subj.diff_offset:
                ans[inf_subj.sem ]['div_zaceths'].append(subj.subject)
